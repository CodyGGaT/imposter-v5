...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'O2': // Jadz Week
				var bg:FlxSprite = new FlxSprite(-577, -145).loadGraphic(Paths.image('o2/O2Background', 'impostor'));
				bg.antialiasing = true;
				bg.scrollFactor.set(1, 1);
				bg.active = false;
				add(bg);

				var fansss = new FlxSprite(-153, 168);
				fansss.frames = Paths.getSparrowAtlas('o2/fansss', 'impostor');
				fansss.animation.addByPrefix('spin', 'fansss instance 10', 24, true);
				fansss.animation.play('spin');
				fansss.antialiasing = true;
				add(fansss);

				var lever:FlxSprite = new FlxSprite(-421, 436).loadGraphic(Paths.image('o2/switch', 'impostor'));
				lever.antialiasing = true;
				lever.scrollFactor.set(1, 1);
				lever.active = false;
				add(lever);
